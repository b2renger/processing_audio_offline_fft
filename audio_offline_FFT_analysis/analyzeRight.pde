...
void analyzeRight() {
  ...
  int totalChunks = (rightChannel.length / fftSize) + 1;
  ...
  for (int chunkIdx = 0; chunkIdx < totalChunks; ++chunkIdx) {
    ...
    int chunkSize = min( rightChannel.length - chunkStartIndex, fftSize );
    ...
    newRow.setString("channel", "RIGHT");
  ...
  String name = cp5.get(Textfield.class, "export_name").getText() +"_" + nbands*10 +"bands"+ "_RIGHT.csv";
